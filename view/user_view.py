...
class CreateFormInput(tk.Toplevel):
    def __init__(self, parent, controller):
        ...
        articles_label = tk.Label(self, text="Articles (comma-separated IDs)")
    ...
    def submit_user_data(self):
        ...
        self.controller.create_user(name, email, articles)
...
class UpdateFormInput(tk.Toplevel):
    def __init__(self, parent, controller):
        ...
        articles_label = tk.Label(self, text="Articles (IDs sseparados por coma)")
        articles_label.pack(pady=5)
        self.articles_input = tk.Entry(self)
        self.articles_input.pack(pady=5)
    ...
    def submit_user_data(self):
        ...
        email = self.email_input.get()
        articles = [article.strip() for article in self.articles_input.get().split(",") if article.strip()]
        self.controller.update_user(id, name, email, articles)
...
class ReplaceFormInput(tk.Toplevel):
    def __init__(self, parent, controller):
        ...
        articles_label = tk.Label(self, text="Artículos (IDs sseparados por coma)")
        articles_label.pack(pady=5)
        self.articles_input = tk.Entry(self)
        self.articles_input.pack(pady=5)
    ...
    def submit_user_data(self):
        ...
        email = self.email_input.get()
        articles = [article.strip() for article in self.articles_input.get().split(",") if article.strip()]
        self.controller.replace_user(id, name, email, articles)
